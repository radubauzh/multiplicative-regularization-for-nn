...
y_min = -5
y_max = 5
experiment_type = 'Multiplicative_Misclass_'
...
# Read λ_mul from command line (or default to 0.05 if not provided)
if len(sys.argv) > 1:
    try:
        λ_mul = float(sys.argv[1])
        print(f"Using provided λ_mul={λ_mul} from command line")
    except ValueError:
        print(f"ERROR: Invalid λ_mul value '{sys.argv[1]}'. Using default λ_mul=0.05")
        λ_mul = 0.05
else:
    λ_mul = 0.05
...
# For multiplicative script: λ_sum is fixed to 0
λ_sum = 0
