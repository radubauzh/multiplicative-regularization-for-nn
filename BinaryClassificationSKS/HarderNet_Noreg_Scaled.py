...
y_min = -5
y_max = 5
experiment_type = 'NoReg_Misclass_'
...
# For no regularization script: both λ_sum and λ_mul are fixed to 0
λ_sum = 0
λ_mul = 0
...
if len(sys.argv) > 1:
    try:
        init_scaling = float(sys.argv[1])
        print(f"Using provided init_scaling={init_scaling} from command line")
    except ValueError:
        print(f"ERROR: Invalid init_scaling value '{sys.argv[1]}'. Using default init_scaling=0.2")
        init_scaling = 0.2
else:
    init_scaling = 0.2
...
if len(sys.argv) > 2:
    try:
        ...
        print(f"ERROR: Invalid seed value '{sys.argv[2]}'. Using default seed=1")
        seed = 1
else:
    seed = 1
...
if len(sys.argv) > 3:
    try:
        misclassification_rate = float(sys.argv[3])
        ...
        print(f"ERROR: Invalid misclassification_rate value '{sys.argv[3]}'. Using default misclassification_rate=0.0")
        misclassification_rate = 0.0
else:
    misclassification_rate = 0.0
...
if len(sys.argv) > 4:
    if sys.argv[4] in ['scaled', 'kaiming', 'normalized']:
        init_method = sys.argv[4]
        print(f"Using provided init_method={init_method} from command line")
    else:
        print(f"ERROR: Invalid init_method value '{sys.argv[4]}'. Must be one of 'scaled', 'kaiming', 'normalized'. Using default init_method='scaled'")
        init_method = 'scaled'
...
for epoch in range(epochs):
    ...
    for X_batch, y_batch in train_loader:
        ...
        # No regularization is applied in this script
        total_loss = loss
